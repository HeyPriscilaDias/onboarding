...
export const getFeedbackScores = (feedback: FeedbackEntry[]): FeedbackScores => {
  const clarity = feedback.find((q) => q.questionId === 6)?.score || null;
  const preparedness = feedback.find((q) => q.questionId === 7)?.score || null;
...
export const getUpdatedFeedback = (
  ...
  return currentStep === 6 ? [newEntry] : [...(existingFeedback || []), newEntry];
};
